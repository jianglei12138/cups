...
read_cups_files_conf(cups_file_t *fp)	/* I - File to read from */
{
  ...
  */
  ...
  while (cupsFileGetConf(fp, line, sizeof(line), &value, &linenum))
  {
	...
	{
          printf("Unknown Group \"%s\" on line %d of %s.", value,
    ...
    else if (!_cups_strcasecmp(line, "PrintcapFormat") && value)
    {
      ...
      */
      ...
      else
      {
          ...
          printf( "Unknown PrintcapFormat \"%s\" on line %d of %s.",
    ...
    else if (!_cups_strcasecmp(line, "Sandboxing") && value)
    {
      ...
      */
      ...
      {
 printf("Unknown Sandboxing \"%s\" on line %d of %s.",
	                value, linenum, CupsFilesFile);
	cupsdLogMessage(CUPSD_LOG_ERROR,
    ...
    else if (!_cups_strcasecmp(line, "SystemGroup") && value)
    {
      ...
      */
      ...
      {
 printf("Unknown SystemGroup \"%s\" on line %d of %s.", value,
	                linenum, CupsFilesFile);
	cupsdLogMessage(CUPSD_LOG_ERROR,
	...
	if (!uid)
	{
 printf( "Will not use User 0 as specified on line %d of %s "
	...
	if (p)
	{
	  if (!p->pw_uid)
	  {
 printf("Will not use User %s (UID=0) as specified on line "
	...
	{
 printf("Unknown User \"%s\" on line %d of %s.",
	                  value, linenum, CupsFilesFile);
	  cupsdLogMessage(CUPSD_LOG_ERROR,
          ...
          if (FatalErrors & CUPSD_FATAL_CONFIG)
            return (0);
             ...
             !_cups_strcasecmp(line, "ServerKey"))
    {
 printf("The \"%s\" directive on line %d of %s is no longer "
